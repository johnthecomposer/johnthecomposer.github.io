  ...
  return {
     ...
     //     if(prp === 'marginLeft' && !this_prp.range.upper){
         if(prp === 'marginLeft'){
          ...
          //    + ' ' + target) + 5;
          var offset = ($(window).outerWidth() - elm.outerWidth()) / 2;
          this_prp.range.lower = -offset;
...
' ' + prp + '; nst % this_prp.resolution = ' + nst + ' % ' + this_prp.resolution + ': ' +
         ...
         if((nst * 10) % (this_prp.resolution * 10) === 0 &&
            ...
            nst <= this_prp.range.upper){
             ...
             // slide title h3 left
             if(prp === 'marginLeft'){
                  //alert('ttl: ' + '#' + image.id + ' ' + target + ' h3')
                  //console.log(ttl)
                  ttl.css('marginRight', this_prp.unit ? nst + this_prp.unit : nst);
             }
         }
       }
       //console.log(this.state.images)
